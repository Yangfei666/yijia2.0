...
export default {
  ...
  methods: {
    ...
    success(obj) {
      ...
      if (index !== 0 || index !== this.courseDaily[this.classroom].item.length) {
    ...
    clickTimeButtom(time, classroom, event) {
      ...
      if (this.classroom != "" && this.classroom != classroom) {
        //说明是选择的第二个时间,但是教室不同,不许这么干
        ...
        if (this.startTime == "" && this.endTime == "") {
          ...
          if (this.CompareDate(time, noNullTime)) {
            ...
            let maxTime =
              parseInt(time.substring(0, 2)) + 2 + time.substring(2);
            if (
              this.CompareDate(noNullTime, minTime) &&
              this.CompareDate(maxTime, noNullTime)
            ) {
      ...
      event.currentTarget.setAttribute(
        "class",
        classStyle == "wer-div2" ? "wer-div" : "wer-div2"
      ); //样式改变放最后
    },
    // 时间比大小
