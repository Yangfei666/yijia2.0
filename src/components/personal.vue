...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          .catch(error => {
            this.$message({message: '对不起,教练信息加载失败', type: "error"});
          });
        } else {
          return false;
        }
    ...
    cardInfoHandle(item){
      if (this.ruleForm.consumer == 'member') {
        let string = item.card_type.CTName+' ('+ (item.isEnabled == 1 ? '已启用' : '已禁用')+'; 到期时间:'+item.eTime+'; 状态:'+ item.State +'';
        if (item.SYCS > 0) {
        ...
        return item.experience_voucher.tkName+' (到期时间:'+item.endTime+'; 剩余次数:'+ item.surplus +')';
      }
    }
  }
};
